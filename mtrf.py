...
# Save path
trf_path = fig_path.replace(paths().plots_path(), paths().save_path())

# Load alphas
alphas_fname = f'Best_alphas.pkl'
if trf_params['alpha'] != 'best':
    alphas = {}
else:
    try:
        alphas = load.var(file_path=trf_path + alphas_fname)
    except:
        alphas = {}
            ...
            for value in trf_params['input_features'][feature]:
                if value == 'mss':
                    ...
                    for value_2 in values:
                        feature_value = f'{feature}-{value}{value_2}'
...
for sub_idx, subject_code in enumerate(exp_info.subjects_ids):
    ...
    if os.path.exists(trf_path + trf_fname) and use_saved_data:
        ...
        alpha = alphas.get(subject_code, trf_params['alpha'])
        rf = functions_analysis.compute_trf(subject=subject, meg_data=meg_data, trial_params=trial_params, trf_params=trf_params, meg_params=meg_params,
                                            features=list(feature_evokeds.keys()), alpha=alpha, use_saved_data=use_saved_data, save_data=save_data,
