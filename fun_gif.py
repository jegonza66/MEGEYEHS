...
subject_code = '16990002'

subj_path = pathlib.Path(os.path.join(mri_path, subject_code))
mri_file_path = list(subj_path.glob('*WIP*.nii'))[0]
...
slice_1 = 160
ax[0].imshow(np.flip(img_data[slice_1, :, :], axis=0), origin='lower', cmap='gray')
...
slice_2 = 100
ax[1].imshow(np.flip(img_data[:, slice_2, :], axis=0), origin='lower', cmap='gray')
...
slice_3 = 80
ax[2].imshow(np.flip(img_data[:, :, slice_3].T), origin='lower', cmap='gray')
...
plt.ioff()
for i in range(img_data.shape[0]):
    plt.imshow(ndimage.rotate(img_data[i,:,:], 0), cmap="gray")
...
with imageio.get_writer(f'{subject_code}_gif.gif', mode='I') as writer:
...
fname = os.path.join(subjects_dir, subject_code, 'surf', 'rh.white')
...
brain = Brain(subject_code, hemi='rh', surf='sphere', size=(800, 600))
...
brain = Brain(subject_code, 'both', 'inflated', subjects_dir=subjects_dir, cortex='low_contrast', background='white',
              size=(800, 600))
