...
for param in param_values.keys():
    ...
    for param_value in param_values[param]:
        ...
        if surf_vol == 'volume' or surf_vol == 'mixed':
            fig_path = paths().plots_path() + (f"Source_Space_{meg_params['data_type']}_TF/" + run_path + f"{model_name}_{surf_vol}_ico{ico}_{int(spacing)}_"
                                               f"{pick_ori}_{bline_mode_subj}_{source_estimation}/")
        else:
            fig_path = paths().plots_path() + (f"Source_Space_{meg_params['data_type']}_TF/" + run_path + f"{model_name}_{surf_vol}_ico{ico}_{pick_ori}_"
        ...
        for subject_code in exp_info.subjects_ids:
            ...
            if plot_individuals:
                fname = f"Power_plotjoint_{subject.subject_id}_{meg_params['regions_id']}_{bline_mode_subj}_{l_freq}_{h_freq}"
                plot_general.tfr_plotjoint_picks(tfr=source_tf, plot_baseline=None, bline_mode=bline_mode_subj, vlines_times=0,
                                                 display_figs=display_figs, save_fig=save_fig, trf_fig_path=fig_path, fname=fname)
        ...
        if plot_ga:
            for i, region in enumerate(region_labels):
                ...
                ga_tf_region = source_tf.copy().pick(source_tf.ch_names[0])
                ga_tf_region.data = GA_stc_data[i]
                ga_tf_region.data = np.expand_dims(GA_stc_data[i], axis=0)

                fname = f'GA_{region}'
                fig = ga_tf_region.plot()[0]
                fig.suptitle(fname)
